...
impl ConfigObject for UpdateCheckTimestamp {
    fn save_config<W: Workspace>(&self, workspace: &W) -> api::error::Result<()> {
        workspace.save_config_object(self)
    }

    fn read_config<W: Workspace>(workspace: &W) -> api::error::Result<Self> {
        Ok(workspace.read_config_object::<Self>().unwrap_or_default())
    }

    fn remove_config<W: Workspace>(_workspace: &W) -> api::error::Result<()> {
        unimplemented!()
    }
